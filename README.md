...
|                       | learning                                                           | 学习任务模型：每日群印象总结、表达/黑话学习与自审、人物事实抽取与自审、表情包 smart 跟发选图都靠它（建议与 judge 同为便宜快速的模型）；**不配置则继承 judge 的配置**；同样支持对象写法覆盖提供商与限额                                                                                                                                                |
...
|                       | person_enabled                                                     | 人物长期记忆单项开关（默认开，另受 `learning.enabled` 总开关约束）：随表达/黑话同一批后台学习抽取「关于群友的稳定事实」并入 `person_fact` 表，回复前把本轮相关人物的画像注入 **L4**（见下文「人物记忆」）。`false` 时学习批次与 L4 组装与引入前逐字节一致                                                                                                              |
|                       | person_fact_scope                                                  | 人物事实作用域（默认 `group`）：`group` 事实只在学它的那个群注入（存储/检索键带群号）；`global` 同一 QQ 号跨群可见——**跨群使用他人信息有社交风险**（A 群聊到的事在 B 群被它接话），想清楚再开                                                                                                              |
|                       | person_fact_max_inject_per_person                                  | 单次回复每人最多注入的画像条数（默认 4）；人数上限 ≤3（触发发送者 → 被 @ 的人 → 被回复消息的发送者）与全块 ≤200 字预算固定不可配                                                                                                                                                                             |
|                       | person_fact_half_life_days / person_fact_min_weight                | 久不命中事实的衰减半衰期（天，默认 30；每过一个半衰期检索权重减半）/ 注入下限（默认 0.25，衰减后低于它不再触达模型；行保留不删，任何一次被注入即刷新强化）                                                                                                                                     |
|                       | person_self_review                                                 | 人物事实可选自审（默认关）：打开后每条候选事实入库前多一次 learning 判定调用，只收明确合适的条目                                                                                                                                                                                                   |
...
|                       | respond_reset_minutes                                              | 主动发言后的「有人接」观察窗（分钟，默认 10）：窗内有人 **@机器人或回复机器人**才算有人接话，退避倍数重置 ×1（普通闲聊不算回应）；窗内没人接，下轮空闲窗口 ×2，封顶 ×8（越没人理越安静）                                                                                                                                                                                                        |
...
四项后台学习能力（配置 `learning` 段，`enabled: false` 整体关闭；LLM 调用统一用 `models.learning` 角色，未配置则继承 `judge`；表达选取的 vector 模式与人物事实的语义近重合并另用 `models.embedding` 角色做文本向量化，见下）。所有学习任务都在后台 asyncio 任务中执行，绝不阻塞每群决策队列；失败只记 warning 日志并跳过本次，不重试、不堆积。
...
- **人物记忆（关于「人」的长期记忆）**：与表达/黑话同一批学习顺带抽取**关于群友的稳定事实**（`person_enabled`，默认开）——身份（学生/职业/所在城市）、长期好恶、群友间人际关系、与 bot 的长期约定；提示词硬约束 + 代码侧隐私硬拦截双保险：一时情绪、玩笑、寒暄、转述的网上的话一律不抽，住址、学校全名、工作单位全称、证件号与联系方式**一律不入库**（防画像变成隐私泄露面）。抽到的事实按人存进 `person_fact` 表（带 count/权重与最近命中时间）；昵称→QQ 号只在本批发言人里**精确**匹配，匹配不到或重名歧义一律丢弃——错把 A 的事实挂到 B 头上不可接受。配置了 `models.embedding` 时入库前与同人已有事实做语义近重合并（cosine > 0.92 视为同一事实，累加 count 不新增行；向量存 `person_fact_embedding` 新表），未配置则退化为文本精确去重。回复前按优先级取本轮相关人物（触发消息发送者 → 被 @ 的人 → 被回复消息的发送者，≤3 人、合计 ≤200 字），每人取衰减后 top `person_fact_max_inject_per_person` 条注入 **L4** 整块画像：「【人物画像-内部参考】关于 小明：- 是大三学生，最近考研 - 喜欢喝蜜雪冰城」，并明确标注**仅供内部理解、不得向群友逐字复述或点名宣读、与当前对话冲突时以当前对话为准**。遗忘用衰减模拟：检索权重 = weight × (1+ln(1+count)) × 0.5^(距上次命中天数 / `person_fact_half_life_days`)，反复学到的更抗遗忘；低于 `person_fact_min_weight` 就不再触达模型（行保留不删，便于审计/复活；任何一次被注入即刷新命中——被使用=被强化）。作用域 `person_fact_scope`：`group`（默认）事实只在学的群里注入；`global` 同一 QQ 号跨群可见——**跨群使用他人信息有社交风险**（A 群聊到的事 B 群被接话），提示词护栏之外请自行斟酌。`person_self_review`（默认关）为可选的入库自审。`person_enabled: false` 时学习批次与 L4 组装和引入本机制前逐字节一致。

数据都在 `candy.db`（`group_impression` / `expressions` / `jargons` / `person_fact` 四表，vector 模式下表达条目的语义向量另存 `expression_embedding` 新表、人物事实的语义向量另存 `person_fact_embedding` 新表）。DEBUG 日志可见学习任务触发与产出、每次回复 L4 注入了哪些表达/黑话（vector 模式还带相似度分数）与人物画像（「群 %d L4 注入人物画像」列出人选与条数）。连续跑几天后应能观察到：L2 稳定携带群印象，回复开始出现从群友身上学到的表达，并在合适时机自然接住「对方之前提过的事」（如问候考得怎么样），且从不把画像原文复读给对方听。
...
- **生成与发送**：复用现有 `generate_reply` 管线，只把 L4 指令层换成自发言变体（「【主动发言】没人问你话，你只是想主动说点什么：{intent}。用完全口语、≤2 句的说法，不要引用消息、不要@任何人……」，既有回复文本一字未动）；表达/黑话注入照常、按 `context_messages`（默认沿用该群 `context_size`）取上下文。生成的文本仍过 AI 味拦截与 `postprocess`（拆条/打字延迟/错别字全保留）逐条发送；连发同样受**打断重想**保护——以决策时刻的消息集为基线，生成中或打字延迟期间进来他人消息时，先问 reply 模型对剩余腹稿放弃/改写/照发（`max_reconsider_per_burst` 预算，与被动链路同源共用 `_make_reconsider`）；成功后写回记忆（is_self）、刷新冷却/间隔/日配额——与被动回复共用同一段记账代码。**只有实际至少发出 1 条正文**才计每群当日上限并开启观察窗，一条都没发出退还配额、什么都不记，与既有规则对齐。
- **退避**：实际发出后进入 `respond_reset_minutes` 的「有人接」观察窗——窗内有人 **@机器人或回复它**才算有人接话，退避倍数重置 ×1；普通闲聊不算回应（没人接自己的话却因别人互聊而重置，会把该安静的时候说得更勤），窗内没人接，下轮空闲窗口 ×2，封顶 ×8：越没人理越安静。
- **风险提示**：主动发言是行为风险最高的功能——它可能在不合适的时机说话。因此**默认关闭**、参数保守（仅当天活跃过的群、每群每日 ≤2、默认沉默、没人接话指数退避），且开关可热重载：`enabled` 改 false 保存立即停止全部待触发心跳。

DEBUG 日志可见排程与决策（「群 %d 空闲评估排在 %.0f 秒后」「入队后来了新消息，放弃本轮」「空闲评估想主动发言：<intent>」「被护栏拦下」「主动发言完成（N 条）」）、观察窗与退避（「主动发言后有人接话（@/回复），退避重置」「主动发言没人接，下轮空闲窗口退避为 ×N」）；INFO 可见心跳启停（「主动发言心跳已启动/已停止」）。
...
4. user·指令层：秒级时间、触发类型等易变信息（含 L4 注入的表达/黑话参考、人物画像、重复回复提醒、临时随机风格与 AI 味拦截重写要求）全部压到最后一层。表达条目无论走加权随机还是 vector 语义召回，注入文案与格式完全相同、都只进这一层，检索结果绝不触碰 L1/L2 前缀。
...
21. **主动发言（心跳）**：保持默认 `proactive.enabled: false` 启动 → 日志无「主动发言心跳已启动」行，行为与引入前完全一致。显式开启并把窗口调短实测：`{ enabled: true, idle_min_seconds: 60, idle_max_seconds: 120 }` 保存热重载 → INFO 出现「主动发言心跳已启动」；在群里聊几句让当天消息数过 `min_today_messages`，然后彻底安静 → 约 1~2 分钟后冒出一句贴合之前话题的短话（DEBUG 可见 `[proactive]` 评估请求带「【潜水时刻】」与 `submit_proactive`、生成请求 L4 带「【主动发言】没人问你话…≤2 句」；绝不应出现「大家好我是…」式开场白），日志「群 %d 主动发言完成」。没人理它 → DEBUG 出现「主动发言没人接，下轮空闲窗口退避为 ×2」（下次冒泡间隔明显变长，×2 递增封顶 ×8）；有人**@它或回复它** → DEBUG「主动发言后有人接话（@/回复），退避重置」（注意：群友互相闲聊不算接话，不会重置退避）。当天第 3 次到点 → DEBUG「今日主动发言已达上限，空闲评估不调模型」（DEBUG 无 `[proactive]` 请求）。把 `enabled` 改 false 保存 → INFO「主动发言心跳已停止（enabled=false，待触发窗口全部清空）」，立刻不再冒泡。
